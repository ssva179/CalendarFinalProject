...
export default function ImportPage() {
    ...
    const handleImport = async () => {
        ...
        startTransition(async () => {
            ...
            const message = result.sourceEventCount === result.importedCount
                ? `Imported ${result.sourceEventCount} event${result.sourceEventCount === 1 ? "" : "s"}.`
                : `Imported ${result.sourceEventCount} calendar event${result.sourceEventCount === 1 ? "" : "s"} as ${result.importedCount} occurrence${result.importedCount === 1 ? "" : "s"}.`;
    ...
    const handleDeleteImported = () => {
        ...
        startTransition(async () => {
            ...
            const message = `Deleted ${result.deletedCount} imported event${result.deletedCount === 1 ? "" : "s"}.`;
            setStatusMessage(message);
            alert(message);
            router.refresh();
        });
        ...
        <>
            ...
            <Container>
                <Card>
                    ...
                    <Typography variant="body1" sx={{ color: "#2f2417", lineHeight: 1.6 }}>
                        Upload a calendar file to add its events. Recurring events are expanded and imported through their end date.
                    </Typography>
                    ...
                    {statusMessage && (
                        <Typography variant="body2" sx={{ color: "#4f3419", fontWeight: 500 }}>
                            {statusMessage}
                        </Typography>
                    )}
