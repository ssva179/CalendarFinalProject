...
function parseProperty(line: string): { name: string; prop: IcsProperty } | null {
...
function parseDateValue(rawValue: string, params: Record<string, string> = {}): Date | null {
...
function parseRRule(rawValue: string): ParsedRRule | null {
    ...
    const supportedKeys = new Set(["FREQ", "INTERVAL", "COUNT", "UNTIL", "BYDAY", "BYMONTHDAY"]);
    ...
    const byDay = values.BYDAY ? values.BYDAY.split(",").map((value) => value.toUpperCase()) : undefined;
    const byMonthDay = values.BYMONTHDAY
        ? values.BYMONTHDAY.split(",").map((value) => Number(value)).filter((value) => Number.isFinite(value))
        : undefined;

    if (byDay?.some((value) => !["SU", "MO", "TU", "WE", "TH", "FR", "SA"].includes(value))) {
        return null;
    }
    ...
    if ((freq === "DAILY" || freq === "WEEKLY") && byMonthDay && byMonthDay.length > 0) {
        return null;
    }
...
function expandRecurringStarts(start: Date, event: ParsedEvent, rule: ParsedRRule): Date[] {
    ...
    if (rule.freq === "WEEKLY") {
        ...
        for (let offset = 0; offset < MAX_RECURRING_OCCURRENCES * 7; offset += 1) {
            ...
            const weeksSinceStart = Math.floor((day.getTime() - start.getTime()) / (7 * MS_IN_DAY));
    ...
    if (rule.freq === "MONTHLY") {
        ...
        for (let monthOffset = 0; monthOffset < MAX_RECURRING_OCCURRENCES; monthOffset += rule.interval) {
    ...
    for (let yearOffset = 0; yearOffset < MAX_RECURRING_OCCURRENCES; yearOffset += rule.interval) {
...
function buildEventDocuments(parsedEvent: ParsedEvent, userEmail: string) {
    const summary = getProperty(parsedEvent, "SUMMARY")?.value?.trim() || "Imported Event";
    const description = getProperty(parsedEvent, "DESCRIPTION")?.value?.replace(/\\n/g, "\n").trim();
    ...
    if (!end) {
        end = startProp.params.VALUE?.toUpperCase() === "DATE" ? addDays(start, 1) : new Date(start);
    }

    const duration = Math.max(0, end.getTime() - start.getTime());
    const notes = description ? [description] : [];
    const starts = ruleProp
        ? expandRecurringStarts(start, parsedEvent, parseRRule(ruleProp.value) ?? (() => {
...
export async function importIcsEvents(icsText: string): Promise<ImportResult> {
    ...
    if (!userEmail) {
        return { importedCount: 0, sourceEventCount: 0, error: "You must be logged in to import events." };
    }

    if (!icsText.trim()) {
        return { importedCount: 0, sourceEventCount: 0, error: "The uploaded file is empty." };
    }

    try {
        const parsedEvents = parseEvents(icsText);
        const builtEvents = parsedEvents.map((parsedEvent) => buildEventDocuments(parsedEvent, userEmail));
        const validSourceEventCount = builtEvents.filter((eventGroup) => eventGroup.length > 0).length;
        const documents = builtEvents.flat();

        if (documents.length === 0) {
            return { importedCount: 0, sourceEventCount: 0, error: "No valid events were found in the file." };
        }
        ...
        const message = error instanceof Error ? error.message : "Failed to import the .ics file.";
...
export async function deleteImportedEvents(): Promise<DeleteResult> {
    ...
    if (!userEmail) {
        return { deletedCount: 0, error: "You must be logged in to delete imported events." };
    }
